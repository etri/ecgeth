...
func (cs *chainSyncer) nextSyncOp() *chainSyncOp {
	...
	//for ec consensus, let's loose the condition in order to deal with fast block production
	//we assume ec will incread td by 1
	//even though this node is 1 block behind, we assume we are in sync, yim

	var peerTD big.Int
	peerTD.Set(op.td)
	peerTD.Sub(&peerTD, big.NewInt(1))

	//log.Trace("TD", "peer org td", op.td.Int64(), "peer new td", peerTD.Int64(), "our td", ourTD.Int64())
	if peerTD.Cmp(ourTD) <= 0 {
		return nil // We're in sync.
	}
	//end ec, by yim

	/*/original code commented by yim
