...
// BroadcastBlockToAll will either propagate a block to its peer
// if the number of connected nodes are small, sending block directly to all peer is ok
// however if # of nodes are large, how can handle it..
//maybe, we need to use the stategy of ethereum, sending block to subset of peer
//and then send the hash to rest of peers
//added by yim, for external consensus
func (h *handler) BroadcastBlockToAll(block *types.Block) {
	hash := block.Hash()
